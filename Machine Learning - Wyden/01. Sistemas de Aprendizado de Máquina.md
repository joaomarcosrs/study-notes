...
Essa situação ilustra um _trade-off_: o animal pode escolher por ==“testar”== novas combinações de ações ótimas em uma sequência de “estados” não realizada anteriormente, ==em busca de uma recompensa maior, mas não imediata== (o chamado _exploration_); ou s==implesmente se ater à recompensa que ele pode obter por meio de uma ação já conhecida== (o chamado _exploitation_).
